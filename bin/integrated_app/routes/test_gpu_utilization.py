...
# Add parent directory to path (integrated_app)
...
def test_nvml_initialization():
    ...
    try:
        ...
        if handle1 is not None:
            ...
            logger.warning("⚠️ NVML handle is None (may not have NVIDIA GPU or pynvml not installed)")
    ...
    try:
        ...
        if util is not None:
            ...
            if 0 <= util <= 100:
                ...
                logger.warning(f"⚠️ Utilization value out of range: {util}")
            return True
        else:
            logger.warning("⚠️ NVML utilization returned None")
            return False
    except Exception as e:
...
def test_gpu_utilization_nvidia_smi():
    ...
    try:
        ...
        if util is not None:
            ...
            logger.info("⚠️ nvidia-smi utilization returned None (may not be available)")
...
def test_thread_safety():
    ...
    try:
        ...
        if len(valid_handles) > 0:
            ...
            logger.warning("⚠️ No valid handles obtained (NVML may not be available)")
            return True
    except Exception as e:
