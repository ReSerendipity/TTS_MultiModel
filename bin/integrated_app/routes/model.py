...
from ..model_manager import (
    load_voxcpm2, unload_model, switch_engine, preload_model, get_preload_status,
    voxcpm_model, current_engine, _gen_tracker, _progress_mgr,
)
...
async def load_model_endpoint(request: Request):
    ...
    try:
        ...
        # Auto-retry on OOM: up to MAX_RETRIES attempts
        last_error = None
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if results:
                    return JSONResponse({"status": "ok", "message": results[-1]})
...
@switch_router.post("/api/switch_engine")
async def switch_engine_endpoint(request: Request, engine: str = Form(...)):
    try:
        ...
        return JSONResponse({"status": "ok", "message": final_status, "engine": current_engine})
    except Exception as e:
        logger.error(f"Engine switch failed: {e}")
        rolled_back_engine = current_engine if current_engine else prev_engine
        rollback_msg = ""
