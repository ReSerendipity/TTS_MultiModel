...
officially support AMD ROCm or Intel XPU backends. ROCM/XPU enum members are
defined for API completeness/forward-compatibility only; no strategy is
registered for them, so using them will fall back to the CPU strategy.
"""
...
class GPUBackend(Enum):
    ...
    # ROCM/XPU 仅作为 API 完整性/前向兼容保留，本项目实际不支持（详见模块文档）
    ROCM = "rocm"  # AMD GPUs (planned, not officially supported)
    XPU = "xpu"  # Intel GPUs (planned, not officially supported)
