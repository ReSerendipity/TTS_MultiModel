...
class FasterQwen3TTS:
    ...
    def __init__(
        ...
        self._voice_prompt_cache_max = 16  # 最大缓存条目数，防止 GPU 显存泄漏
    ...
    def _infer_sample_rate(base_model) -> int:
        ...
        if speech_tokenizer is not None:
            get_sr_fn = getattr(speech_tokenizer, "get_output_sample_rate", None)
            if callable(get_sr_fn):
                try:
                    sample_rate = get_sr_fn()
                except Exception:
                    pass
    ...
    def _prepare_generation(
        ...
        if cache_key in self._voice_prompt_cache:
            ...
            # LRU 淘汰：超过最大缓存数时删除最早的条目
            if len(self._voice_prompt_cache) > self._voice_prompt_cache_max:
                oldest_key = next(iter(self._voice_prompt_cache))
                del self._voice_prompt_cache[oldest_key]
        else:
    ...
    def generate_custom_voice(
        ...
        if self.model.model.tts_model_size == "0b6":
            instruct = None
