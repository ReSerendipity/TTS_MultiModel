...
### 2.3 改动清单（`bin/integrated_app/history_db.py`）
...
### 3.2 改动（`bin/integrated_app/history_db.py`）
...
### 4.1 改动（`bin/integrated_app/history_db.py`）
...
### 5.2 改动（`bin/integrated_app/audio_processing.py`）
...
```powershell
.\WPy64-312101\python\python.exe -m ruff check bin/integrated_app/history_db.py bin/integrated_app/audio_processing.py --statistics
```
...
| 文件 | 改动类型 | 影响范围 |
|------|----------|----------|
| `bin/integrated_app/history_db.py` | FTS5 + keyset + 迁移去重 | 搜索/分页/迁移（向后兼容） |
| `bin/integrated_app/audio_processing.py` | enhance_audio 懒拷贝 | 内存优化（向后兼容） |
