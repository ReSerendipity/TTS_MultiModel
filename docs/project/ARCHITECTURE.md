...
| 模块 | 位置 | 职责 |
|------|------|------|
| `engine_interface.py` | `app/integrated_app/` | `TTSEngine` Protocol + `InMemoryEngineRegistry` |
| `model_registry.py` | `app/integrated_app/` | 全局引擎状态单例 |
| `model_manager.py` | `app/integrated_app/` | 模型加载/卸载/切换管理（RLock 串行） |
| `generation.py` | `app/integrated_app/` | 文本分割、音频合并、WAV 保存 |
| `gpu_backend.py` | `app/integrated_app/` | GPU 后端抽象（CUDA/MPS/CPU） |
| `config.py` / `config_models.py` | `app/integrated_app/` | 配置加载/验证/原子写入 |
| `history_db.py` | `app/integrated_app/` | SQLite 异步历史记录 |
| `mcp_server.py` | `app/integrated_app/` | MCP Server（Model Context Protocol） |
| `monitor.py` | `app/integrated_app/` | HealthMonitor（显存/利用率/泄漏检测） |
| `task_queue.py` | `app/integrated_app/` | 异步生成任务队列 |
| `watermark.py` / `audio_watermark.py` | `app/integrated_app/` | FFT 频域水印嵌入/检测 |
| `ras_sampling.py` | `app/integrated_app/` | RAS 重复感知采样（Fish Speech 借鉴） |
| `bad_case_retry.py` | `app/integrated_app/` | 坏案例重试（参数调整 + 种子轮换） |
