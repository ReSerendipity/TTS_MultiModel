...
| 文件 | 用途 |
|------|------|
| `bin/integrated_app/engines/gptsovits_engine.py` | GPT-SoVITS 进程内引擎适配器（TTSEngine 协议） |
| `bin/integrated_app/engines/dotstts_engine.py` | dots.tts 进程内引擎适配器（TTSEngine 协议） |
| `bin/integrated_app/routes/generate/generic/__init__.py` | 通用引擎生成路由聚合 |
| `bin/integrated_app/routes/generate/generic/clone.py` | `POST /api/generate/generic/clone` 端点 |
| `bin/integrated_app/templates/tabs/gptsovits_clone.html` | GPT-SoVITS 克隆 Tab 模板 |
| `bin/integrated_app/templates/tabs/dotstts_clone.html` | dots.tts 克隆 Tab 模板 |
...
| 序号 | 文件 | 改动概述 |
|------|------|----------|
| 3 | `bin/integrated_app/config.py` | 新增 `GPTSOVITS_MODEL_PATH` / `DOTSTTS_MODEL_PATH` |
| 4 | `bin/integrated_app/engine_interface.py` | 在 `_register_builtin_engines()` 中懒导入注册两引擎 |
| 5 | `bin/integrated_app/model_registry.py` | 新增通用引擎容器 `_engines` 及相关方法 |
| 6 | `bin/integrated_app/model_manager.py` | 新增 `_load_generic_engine` + 通用分支 |
| 7 | `bin/integrated_app/routes/model.py` | `/load` 端点支持通用引擎 |
| 8 | `bin/integrated_app/routes/generate/__init__.py` | 导入 `generic` 包 |
| 9 | `bin/integrated_app/routes/tabs.py` | Tab 注册 + 字符上限 + persona |
| 10 | `bin/integrated_app/templates/base.html` | 模型切换按钮 + 侧边栏 |
| 11 | `bin/integrated_app/locales/zh.json` + `en.json` | i18n 键值 |
...
> **验证**：确认 `pretrained_models/GPT-SoVITS/` 目录下存在 `.ckpt`、`.pth` 文件
...
# 或手动下载后放入以下目录：
pretrained_models/dots.tts/
└── (dots.tts-soar 权重快照文件...)
```

> **验证**：确认 `pretrained_models/dots.tts/` 目录非空，包含模型权重文件。
...
**文件**：`bin/integrated_app/config.py`
...
**文件**：`bin/integrated_app/engine_interface.py`
...
**关键参数说明**：
- `lazy_module`：格式 `"module_path:ClassName"`，路径相对于 `bin/integrated_app/`。
...
**文件**：`bin/integrated_app/model_registry.py`
...
**文件**：`bin/integrated_app/routes/model.py`
...
**文件**：`bin/integrated_app/routes/generate/__init__.py`
...
**文件**：`bin/integrated_app/routes/tabs.py`
...
在 `bin/integrated_app/locales/zh.json` 中，"tab_indextts2_clone" 行之后插入：
...
在 `bin/integrated_app/locales/en.json` 中，"tab_indextts2_clone" 行之后插入：
...
> **验证**：`python -c "import json; json.load(open('bin/integrated_app/locales/zh.json',encoding='utf-8')); json.load(open('bin/integrated_app/locales/en.json',encoding='utf-8')); print('JSON valid')"`
...
- 下载到 `pretrained_models/<engine>/` 目录
- 支持断点续传，重复运行不会重复下载
- 下载完成后自动校验必需文件是否存在
...
1. 启动应用：`python bin/clean_launch.py`（或 `start.bat`）
...
| 文件路径 | 说明 |
|----------|------|
| `bin/integrated_app/engines/gptsovits_engine.py` | GPT-SoVITS 进程内引擎适配器 |
| `bin/integrated_app/engines/dotstts_engine.py` | dots.tts 进程内引擎适配器 |
| `bin/integrated_app/routes/generate/generic/__init__.py` | 通用引擎路由聚合 |
| `bin/integrated_app/routes/generate/generic/clone.py` | `POST /api/generate/generic/clone` |
| `bin/integrated_app/templates/tabs/gptsovits_clone.html` | GPT-SoVITS 克隆 Tab 模板 |
| `bin/integrated_app/templates/tabs/dotstts_clone.html` | dots.tts 克隆 Tab 模板 |
| `scripts/download_gptsovits.py` | GPT-SoVITS 权重下载脚本 |
| `scripts/download_dotstts.py` | dots.tts 权重下载脚本 |
...
| 文件路径 | 改动概述 | 涉及步骤 |
|----------|----------|----------|
| `bin/integrated_app/config.py` | 新增 2 个路径常量 | 3 |
| `bin/integrated_app/engine_interface.py` | 懒导入注册 2 个引擎 | 4 |
| `bin/integrated_app/model_registry.py` | 新增通用容器 + 5 个方法 + 4 个扩展 | 5 |
| `bin/integrated_app/model_manager.py` | 新增导入 + 泛化校验 + 通用加载/卸载/回滚 | 6 |
| `bin/integrated_app/routes/model.py` | `/load` 端点支持通用引擎 | 7 |
| `bin/integrated_app/routes/generate/__init__.py` | 导入 generic 包 | 8 |
| `bin/integrated_app/routes/tabs.py` | Tab 注册 + 字符上限 + persona | 9 |
| `bin/integrated_app/templates/base.html` | 模型按钮 + 侧边栏区块 | 10 |
| `bin/integrated_app/locales/zh.json` + `en.json` | 7 个 i18n 键值 | 11 |
