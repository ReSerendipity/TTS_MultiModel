...
即"升到 4.57 就能顺手关掉 4 条 ReDoS"这条路，代价是**产品两个引擎直接不可用**。所以
`pyproject.toml` / `requirements.txt` 里的 `transformers>=4.57.0` 被改回
`>=4.52.1,<4.53`，并把上面的实测数字写进注释：上界不是保守猜测，是被引擎真实约束钉住的。
