...
class TaskCheckpoint:
    ...
    def resume_state(self, task_id: str) -> dict[str, Any] | None:
        """加载未完成 checkpoint 并返回续跑所需状态。

        若 checkpoint 不存在或已完成（completed >= total），返回 None。
        已完成的 checkpoint 会被自动清理（删除文件）。

        Args:
            task_id: 任务唯一标识符。

        Returns:
            checkpoint 数据字典（含 remaining / config / engine 等），
            不存在或已完成时返回 None。
        """
        ...
        if completed >= total or not data.get("remaining"):
    ...
    def list_resumable(self) -> list[dict[str, Any]]:
        """列出可续跑的 checkpoint，并清理已完成/无效的文件。

        与 ``list_checkpoints`` 的区别：本方法会删除已完成（completed >= total）
        或 remaining 为空的 checkpoint 文件，仅返回真正可续跑的条目。

        Returns:
            可续跑的 checkpoint 数据列表。
        """
