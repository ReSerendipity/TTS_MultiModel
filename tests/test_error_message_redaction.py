"""`routes/model.py::_safe_error_message` 的脱敏覆盖面。

回归动机（2026-09-20 复核 py/stack-trace-exposure 时发现）：S-R6 的
`_SENSITIVE_PATH_PATTERN` 脱敏只用在 `OSError` 与兜底分支上，而
...
from integrated_app.routes.model import _safe_error_message  # noqa: E402
...
class TestDomainErrorsAreRedacted:
    ...
    def test_posix_path_never_escapes(self, exc_cls, label):
        out = _safe_error_message(exc_cls(f"读取 {POSIX_PATH} 失败"))
    ...
    def test_windows_path_never_escapes(self, exc_cls):
        out = _safe_error_message(exc_cls(f"加载失败：{WIN_PATH} 缺失"))
        _assert_redacted(out, WIN_PATH)


class TestOtherBranchesUnchanged:
    def test_file_not_found_is_generic_message(self):
        assert _safe_error_message(FileNotFoundError(POSIX_PATH)) == "文件不存在或已被删除"

    def test_permission_error_is_generic_message(self):
        assert "权限不足" in _safe_error_message(PermissionError(POSIX_PATH))

    def test_oserror_still_redacts(self):
        _assert_redacted(_safe_error_message(OSError(f"IO 失败 {POSIX_PATH}")), POSIX_PATH)

    def test_unknown_exception_still_redacts(self):
        _assert_redacted(_safe_error_message(RuntimeError(f"boom {POSIX_PATH}")), POSIX_PATH)

    def test_none_returns_unknown(self):
        assert _safe_error_message(None) == "未知错误"

    def test_long_message_truncated(self):
        out = _safe_error_message(RuntimeError("x" * 900))
